...
/// [sideEffectBlocTest] will handle asserting that the `bloc` emits the [expect]ed
/// states (in order) after [act] is executed.
/// [sideEffectBlocTest] also handles ensuring that no additional states are emitted
...
/// ```
///
/// [sideEffectBlocTest] can optionally be used with a seeded state.
///
/// ```dart
/// sideEffectBlocTest(
...
/// ```
///
/// [sideEffectBlocTest] can also be used to [skip] any number of emitted states
...
/// ```
///
/// [sideEffectBlocTest] can also be used to wait for async operations
...
/// ```
///
/// [sideEffectBlocTest] can also be used to [verify] internal bloc functionality.
///
/// ```dart
/// sideEffectBlocTest(
///   'CounterBloc emits [1] when increment is added',
...
/// ```
///
/// **Note:** when using [sideEffectBlocTest] with state classes which don't override
...
@isTest
void sideEffectBlocTest<B extends BlocBase<State>, State, SideEffect>(
  String description, {
  ...
  dynamic Function()? expectSideEffects,
  ...
  test.test(
    description,
    () async {
      await testBloc<B, State, SideEffect>(
        setUp: setUp,
        build: build,
        seed: seed,
        act: act,
        ...
        skip: skip,
        expect: expect,
        expectSideEffects: expectSideEffects,
        verify: verify,
        errors: errors,
        tearDown: tearDown,
      );
...
@visibleForTesting
Future<void> testBloc<B extends BlocBase<State>, State, SideEffect>({
  required B Function() build,
  FutureOr<void> Function()? setUp,
  State Function()? seed,
  dynamic Function(B bloc)? act,
  ...
  try {
    await _runZonedGuarded(() async {
      ...
      final sideEffectSubscription = (bloc as SideEffectProvider<SideEffect>).sideEffects.listen(sideEffects.add);
      ...
      if (expectSideEffects != null) {
        final dynamic expected = await expectSideEffects();
        shallowEquality = '$sideEffects' == '$expected';
        try {
          test.expect(sideEffects, test.wrapMatcher(expected));
        } on test.TestFailure catch (e) {
          if (shallowEquality || expected is! List<State>) rethrow;
          final diff = _diff(expected: expected, actual: sideEffects);
          final message = '${e.message}\n$diff';
          // ignore: only_throw_errors
          throw test.TestFailure(message);
        }
      ...
      await sideEffectSubscription.cancel();
